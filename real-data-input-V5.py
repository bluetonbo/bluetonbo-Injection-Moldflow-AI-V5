...
# =================================================================
# 1. 사이드바 및 데이터 로드 (생략 가능하나 구조 유지를 위해 포함)
# =================================================================
with st.sidebar:
    st.header("📂 데이터 로드")
    u1 = st.file_uploader("1. UI 초기 조건", type=['csv','xlsx'])
    ...
    if st.button("🚀 AI 모델 학습 및 초기화", use_container_width=True):
        ...
        if df_i is not None and (df_v is not None or df_r is not None):
            ...
            st.session_state.update({
                ...
                'global_process_vars': vars_list, 'ui_display_vars': [c for c in df_i.columns if c != TARGET_VAR]
            })
            
            init_row = df_i.iloc[0].to_dict()
            for v in vars_list:
                v_min, v_max = int(np.floor(df_comb[v].min())), int(np.ceil(df_comb[v].max()))
                if v_min == v_max: v_max = v_min + 1
                st.session_state['global_bounds'][v] = (v_min, v_max)
                st.session_state['current_inputs'][v] = int(init_row.get(v, v_min))
            st.rerun()
...
if st.session_state['model']:
    ...
    with t1:
        ...
        for i, var in enumerate(st.session_state['ui_display_vars']):
            with cols[i % 3]:
                ...
                st.session_state['current_inputs'][var] = st.slider(
                    f"{var}", b_min, b_max, int(st.session_state['current_inputs'].get(var, b_min)), 
        ...
        def calculate_risk(input_vals_list):
            ...
            # 수식 변경: AI 확률을 베이스로 유지하고, 전문가 신뢰성만큼 패널티(기준 위반) 가중치를 부여
            # 이제 신뢰성이 100%여도 AI 확률이 기본적으로 반영됩니다.
        ...
        if st.session_state['last_res_val'] is not None:
            ...
            if st.session_state['last_opt_df'] is not None:
                st.success("✨ AI 추천 최적 공정 조건 (기준값 및 데이터 종합 판단)")
